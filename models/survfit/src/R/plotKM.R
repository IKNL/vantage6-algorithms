plotKM <- function(master,plotCI=F){
    ...
    for(i in 1:length(temp.master)){
        ...
        if(plotCI){
            lines(temp.master[[i]]$times,temp.master[[i]]$upper,type = 's',col=i,lty=2)
            lines(temp.master[[i]]$times,temp.master[[i]]$lower,type = 's',col=i,lty=2)
        }
    }
    if(length(temp.master)>1){
        legend('topright',legend = names(temp.master),lty = 1,col = 1:length(temp.master))}
}
