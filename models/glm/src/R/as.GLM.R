#' Convert the federated GLM output to a GLM object
#'
#' @param obj output of the federated GLM
#' @param data ...
#'
#' @return GLM object
#'
as.GLM <- function(obj, data=NULL) { #nolint
  ...
  if (!missing(family)) {
    ...
    out$null.deviance <- obj$null.deviance #nolint
    out$aic <- 1
    out$iter <- obj$iter
    out$df.null <- obj$nobs - 1 #nolint
    out$df.residual <- obj$nobs - out$rank #nolint
    ...
    out$df.residual <- nrow(data) - out$rank #nolint
    out$model <- data
  }
  out
}